...
import ru.georgeee.android.colloquium2.handlers.impl.LoadSubjectsCommand;
import ru.georgeee.android.colloquium2.model.Subject;
import ru.georgeee.android.colloquium2.service.SFCommandExecutorService;
...
public class SFServiceHelper {
    ...
    public int loadMarks(Subject subject) {
        final int requestId = createId();
        Intent i = createIntent(context, new LoadMarksCommand(subject), requestId);
        return runRequest(requestId, i);
    }

    public int loadSubjects() {
        final int requestId = createId();
        Intent i = createIntent(context, new LoadSubjectsCommand(), requestId);
        return runRequest(requestId, i);
    }
